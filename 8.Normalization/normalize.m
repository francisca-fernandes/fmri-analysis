...
global defaults;
file_fun = '/Users/francisca/Documents/Data/20180818_182309_FFF_etomidate3rdform_1_1/16/Processed/waiImage_16_0001.nii'; % One functional image that aligns all the others!
file_static  = '/Users/francisca/Documents/Data/Atlas_ROI/P56_Atlas_different.nii';  % RARE/Atlas that was selected to align all animals to
nrepetitions = 340;
...
file_mov = {
    '/Users/francisca/Documents/Data/20180818_182309_FFF_etomidate3rdform_1_1/6/pdata/1/cImage_0001.nii'
    };

file_anat = {
    '/Users/francisca/Documents/Data/20180818_182309_FFF_etomidate3rdform_1_1/6/pdata/1/Image_0001.nii'
    };
...
%     dnrm.write.bb(2,3) = -bbmov(1,3);      % Also needed if using 2012 brain atlas
    dnrm.write.bb(1,3) = bbmov(1,3)+1;
    dnrm.write.bb(2,3) = bbmov(2,3)+1;
    ...
    V_mask_anat = spm_vol(spm_select('FPList',fileparts(fileparts(fileparts(file_mov{i}))),'^brain_mask.nii'));
    spm_write_sn(V_mask_anat,[fileparts(file_mov{i}) filesep 'affreg1norm.mat'], dnrm.write);
...
%     dnrm.write.bb(2,3) = -bbfun(1,3);      % Also needed if using 2012 brain atlas
    dnrm.write.bb(1,3) = bbfun(1,3)+1;
    dnrm.write.bb(2,3) = bbfun(2,3)+1;

    for k = 1:no_folders(i) 
        out_folder = proc{m};
        V_im = spm_vol(spm_select('FPList',out_folder,['^w' search_name '.*\d\d\d\d.nii']));
        for j=1:nrepetitions
            spm_write_sn(V_im(j),[fileparts(file_mov{i}) filesep 'affreg1norm.mat'], dnrm.write);
        end
        V_mean = spm_vol(spm_select('FPList',out_folder,['^wmean' search_name '.*\d\d\d\d.nii']));
        spm_write_sn(V_mean,[fileparts(file_mov{i}) filesep 'affreg1norm.mat'], dnrm.write);
        V_mean = spm_vol(spm_select('FPList',out_folder,['^wcmean' search_name '.*\d\d\d\d.nii']));
        spm_write_sn(V_mean,[fileparts(file_mov{i}) filesep 'affreg1norm.mat'], dnrm.write);
        V_mask = spm_vol(spm_select('FPList',[fileparts(proc{m}) filesep 'Masks'],'^wbrain_mask.nii'));
        spm_write_sn(V_mask,[fileparts(file_mov{i}) filesep 'affreg1norm.mat'], dnrm.write);
        m=m+1;
    end
end