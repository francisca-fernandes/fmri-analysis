...
if apply_to_mean_image
    for m=1:length(scan_folder)
        ...
        % Load mask
        m = spm_vol([main_folder filesep study_folder filesep num2str(scan_f) filesep 'Masks' filesep name_mask '.nii']);
...
if apply_to_all_functional_images
    for m=1:length(scan_folder)
        ...
        for i = 1:nrepetitions
            file = fullfile(input_folder,[search_name '_' num2str(scan_f) '_' num2str(i,'%04d') '.nii']);
            V = spm_vol(file);
        ...
        V_out = V;
        for i = 1:nrepetitions
            V_out.fname = fullfile(input_folder,['c' search_name '_' num2str(scan_f) '_' num2str(i,'%04d') '.nii']);
