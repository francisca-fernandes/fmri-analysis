...
for u=1:length(file_static)
    ...
    for m = 1:no_folders(u) 
        ...
        transf_matrix=inv(spm_matrix(x));
        save([proc{k} filesep 'coreg_matrix.mat'],'transf_matrix')
        
        image_fun_new = image_fun;
        image_fun_new.fname = [proc{k} filesep 'wcmean' search_name '_' num2str(func_exps(k)) '_0001.nii'];
        image_fun_new.mat = spm_matrix(x)\image_fun.mat;
        spm_write_vol(image_fun_new,spm_read_vols(image_fun));
        
        out_folder  =   [fileparts(fileparts(fileparts(file_fun))) filesep num2str(func_exps(k)) filesep 'Processed'];
        V_im        =  spm_vol(spm_select('FPList',out_folder,['^' search_name '.*\d\d\d\d.nii']));
        for i=1:nrepetitions
            V_im_new = V_im(i);
            V_im_new.fname = [proc{k} filesep 'w' search_name '_' num2str(func_exps(k)) '_' sprintf('%04d', i) '.nii'];
            V_im_new.mat = spm_matrix(x)\V_im(i).mat;
            spm_write_vol(V_im_new,spm_read_vols(V_im(i)));
        end
        V_mean      =  spm_vol(spm_select('FPList',out_folder,['^mean' search_name '.*\d\d\d\d.nii']));
        V_mean_new = V_mean;
        V_mean_new.fname = [proc{k} filesep 'wmean' search_name '_' num2str(func_exps(k)) '_0001.nii'];
        V_mean_new.mat = spm_matrix(x)\V_mean.mat;
        spm_write_vol(V_mean_new,spm_read_vols(V_mean));
        
        V_mask      =  spm_vol(spm_select('FPList',[fileparts(fileparts(file_fun)) filesep 'Masks'],'^brain_mask.nii'));
        V_mask_new = V_mask;
        V_mask_new.fname = [fileparts(fileparts(file_fun)) filesep 'Masks' filesep 'wbrain_mask.nii'];
        V_mask_new.mat = spm_matrix(x)\V_mask.mat;
        spm_write_vol(V_mask_new,spm_read_vols(V_mask));
